...
export const getSelectNotes = createFeatureSelector<JobState>('notes');

export const notesSelector = createSelector(getSelectNotes, (state: JobState)=>state.notes); 
...
createSelector(getSelectJob, (state: JobState) => {
  ...
  export const selectStatusById = (statusId: number) => createSelector(getSelectJob, (state: JobState)=>state.statuses.find(s => s.id === statusId)?.name);
